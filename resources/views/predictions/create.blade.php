...
@section('content')
    <div class="m-auto w-4/8 py-24">
        ...
        <h1 class="text-4xl uppercase bold">Make a prediction!</h1>
        </div>
    </div>
        ...
        <form action="/predictions" method="POST">
            ...
            <div class="block align-items-center float-left">
                ...
                <span class="float-left">
                    <h2 class="text-purple-600 text-2xl font-bold text-center">
                        {{ strtoupper($name) }}
                    </h2>
                    @foreach ($listOfDrivers as $driver)
                        ...
                        <label class="form-check-label inline-block text-gray-800" for="{{ $name }}{{ $driver }}">
                            ...
                            id="{{ $name }}{{ $driver }}" 
                            value="{{ $driver }}">
                            <img src="{{ asset('storage/'.$driver.'.png') }}" width="300">
                        </label>
                        <br>
                    @endforeach
                    </span>
